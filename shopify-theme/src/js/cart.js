...
class CartRemoveButton extends HTMLElement {
    constructor() {
        ...
        this.addEventListener('click', (event) => {
            ...
            if (this.dataset.customId) {
                const productAddons = cartItems.querySelectorAll(
                    `input[data-custom-id='${this.dataset.customId}']`
                );
                let updates = {};
                productAddons.forEach((addon) => {
                    const key = addon.dataset.key;
                    updates = { ...updates, [key]: 0 };
                });
...
class CartItems extends HTMLElement {
    ...
    mergeAddons() {
        const parents = Array.from(this.querySelectorAll('tr[isparent]'));
        ...
        parents.forEach((parent) => {
            ...
            const addons = Array.from(
                this.querySelectorAll(
                    `tr[data-custom-id='${customId}']:not([isparent])`
                )
            );

            const addonTotal = addons
                .map((addon) => Number(addon.dataset.price))
                .reduce((partialSum, a) => partialSum + a, 0);

            parent.querySelectorAll('.price').forEach((priceElement) => {
                priceElement.innerHTML = formatMoney(price + addonTotal);
            });
        });
    }
    ...
    updateQuantity(line, quantity, name, updates) {
        ...
        const url = line ? routes.cart_change_url : routes.cart_update_url;
